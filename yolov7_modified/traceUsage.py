...
def detect(opt, save_img=False):
    ...
    gpu = pnv.nvmlDeviceGetHandleByIndex(0)
...
class Monitor(Thread):
    ...
    def run(self):
        while not self.stopped:
            # code here
            pyn.nvmlDeviceGetUtilizationRates(gpu)
            #pyn.nvmlDeviceGetMemoryInfo(gpu)
            pyn.nvmlDeviceGetPcieThroughput(gpu, pyn.NVML_PCIE_UTIL_COUNT)
            pyn.nvmlDeviceGetPowerUsage(gpu)
            pyn.nvmlDeviceGetTotalEnergyConsumption(gpu)
            pyn.nvmlDeviceGetTemperature(gpu, pyn.NVML_TEMPERATURE_GPU)
            report.append()
